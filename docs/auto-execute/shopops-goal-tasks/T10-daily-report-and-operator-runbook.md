# Task T10 - daily-report-and-operator-runbook
...
| Task 类型 | 文档交接 |
| 主验收面 | 日报生成、Windows 运行手册、环境变量说明 |
| 为什么选择这个模板 | 本任务的主要目标是 日报生成、Windows 运行手册、环境变量说明，与 `TPL-DOCS-HANDOFF` 的验收面一致。 |
| 覆盖对象 | REQ-013,REQ-014 |
...
| 独立任务模板文件 | `C:\Users\linyanhui\.codex\skills\task-auto-execute\references\templates\TPL-DOCS-HANDOFF.md` |

## Codex Exec
```powershell
Set-Location -LiteralPath "D:\lyh\agent\agent-frame\ShopOps"
codex exec "Use auto-execute. Execute only D:\lyh\agent\agent-frame\ShopOps\docs\auto-execute\shopops-goal-tasks\T10-daily-report-and-operator-runbook.md. Treat this as one fresh task boundary. Do not stop after planning. Implement, test, repair, write result JSON, write HANDOFF, and exit. If blocked, classify the blocker in durable files and route to repair; do not end with only chat text."
```

## 实现功能
补齐日报 upsert、README、.env.example、千牛 CDP/飞书配置手册。
...
## 允许改动范围
README.md; .env.example; shopops/services/daily_report_service.py; tests/**; docs/auto-execute/**
...
## 验收标准
- 覆盖对象 `REQ-013,REQ-014` 的实现或验证结果必须落到 durable files。
- 本任务结束必须写 `docs/auto-execute/results/T10.json`。
- 本任务结束必须写 `docs/auto-execute/latest/T10-HANDOFF.md`。
- result JSON 必须包含 `task_pack_id: "shopops-goal-2026-06-03"`、`task_id: "T10"`、`status`、`evidence`、`blockers`。
...
| 面 | 标准 | 证据路径 |
| --- | --- | --- |
| 需求 | `REQ-013,REQ-014` 每一项有实现/缺口判定 | `docs/auto-execute/results/T10.json` |
| 数据 | 相关表字段、unique_key、upsert、pending replay、readback 有证据或 blocker | `docs/auto-execute/external-data/T10/` |
| 测试 | 相关命令运行并记录输出，失败则本任务内修复或写 repair | `docs/auto-execute/logs/T10/` |
| 安全 | 不泄露密钥，不触碰真实投放副作用 | `docs/auto-execute/results/T10.json` |

## 测试与证据
```powershell
pytest daily report; docs review。
```

## Dependency And Resume Gate
| 依赖 | 必须存在 | 不满足时处理 |
| --- | --- | --- |
| T09 | 前置 result JSON 和 HANDOFF，`none` 除外 | 读取 blockers/repair queue；不得猜测前置成功 |
...
## 输出文件
docs/auto-execute/results/T10.json; README.md
