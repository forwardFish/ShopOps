...
| Task 类型 | 编排 |
| 主验收面 | 串行 fresh codex exec 编排与续跑 |
| 为什么选择这个模板 | 本任务的主要目标是 串行 fresh codex exec 编排与续跑，与 `TPL-ORCH-T00` 的验收面一致。 |
| 覆盖对象 | ALL |
...
| 独立任务模板文件 | `C:\Users\linyanhui\.codex\skills\task-auto-execute\references\templates\TPL-ORCH-T00.md` |

## Codex Exec
```powershell
Set-Location -LiteralPath "D:\lyh\agent\agent-frame\ShopOps"
codex exec "Use auto-execute. Execute only D:\lyh\agent\agent-frame\ShopOps\docs\auto-execute\shopops-goal-tasks\T00-omx-auto-execute-orchestrator.md. Treat this as one fresh task boundary. Do not stop after planning. Implement, test, repair, write result JSON, write HANDOFF, and exit. If blocked, classify the blocker in durable files and route to repair; do not end with only chat text."
```

## 实现功能
检查新任务包、历史结果、blockers、repair queue，并一次只启动一个后续任务。
...
- 本任务结束必须写 `docs/auto-execute/results/T00.json`。
- 本任务结束必须写 `docs/auto-execute/latest/T00-HANDOFF.md`。
- result JSON 必须包含 `task_pack_id: "shopops-goal-2026-06-03"`、`task_id: "T00"`、`status`、`evidence`、`blockers`。
...
| 面 | 标准 | 证据路径 |
| --- | --- | --- |
| 需求 | `ALL` 每一项有实现/缺口判定 | `docs/auto-execute/results/T00.json` |
| 数据 | 相关表字段、unique_key、upsert、pending replay、readback 有证据或 blocker | `docs/auto-execute/external-data/T00/` |
| 测试 | 相关命令运行并记录输出，失败则本任务内修复或写 repair | `docs/auto-execute/logs/T00/` |
| 安全 | 不泄露密钥，不触碰真实投放副作用 | `docs/auto-execute/results/T00.json` |

## 测试与证据
```powershell
PowerShell 逐个启动后续 task；等待退出；校验 result JSON/HANDOFF/log。
```

## Dependency And Resume Gate
| 依赖 | 必须存在 | 不满足时处理 |
| --- | --- | --- |
| none | 前置 result JSON 和 HANDOFF，`none` 除外 | 读取 blockers/repair queue；不得猜测前置成功 |
...
## 输出文件
docs/auto-execute/results/T00.json; docs/auto-execute/latest/T00-HANDOFF.md
