# Task T06 - qianniu-promotion-center-cost-crawler
...
| Task 类型 | 爬虫管线 |
| 主验收面 | 千牛 PC 推广中心只读花费采集 |
| 为什么选择这个模板 | 本任务的主要目标是 千牛 PC 推广中心只读花费采集，与 `TPL-CRAWLER-PIPELINE` 的验收面一致。 |
| 覆盖对象 | REQ-004,REQ-010 |
...
codex exec "Use auto-execute. Execute only D:\lyh\agent\agent-frame\ShopOps\docs\auto-execute\shopops-goal-tasks\T06-qianniu-promotion-center-cost-crawler.md. Treat this as one fresh task boundary. Do not stop after planning. Implement, test, repair, write result JSON, write HANDOFF, and exit. If blocked, classify the blocker in durable files and route to repair; do not end with only chat text."
```

## 实现功能
实现/验证推广中心页面只读“花费”，禁止渠道拆分和投放修改，失败不写 0。
...
## 允许改动范围
shopops/collectors/taobao_promotion_crawler.py; shopops/services/browser_service.py; tests/**; docs/auto-execute/**
...
## 验收标准
- 覆盖对象 `REQ-004,REQ-010` 的实现或验证结果必须落到 durable files。
- 本任务结束必须写 `docs/auto-execute/results/T06.json`。
- 本任务结束必须写 `docs/auto-execute/latest/T06-HANDOFF.md`。
- result JSON 必须包含 `task_pack_id: "shopops-goal-2026-06-03"`、`task_id: "T06"`、`status`、`evidence`、`blockers`。
...
| 面 | 标准 | 证据路径 |
| --- | --- | --- |
| 需求 | `REQ-004,REQ-010` 每一项有实现/缺口判定 | `docs/auto-execute/results/T06.json` |
| 数据 | 相关表字段、unique_key、upsert、pending replay、readback 有证据或 blocker | `docs/auto-execute/external-data/T06/` |
| 测试 | 相关命令运行并记录输出，失败则本任务内修复或写 repair | `docs/auto-execute/logs/T06/` |
| 安全 | 不泄露密钥，不触碰真实投放副作用 | `docs/auto-execute/results/T06.json` |

## 测试与证据
```powershell
pytest promotion crawler; CDP live probe。
```

## Dependency And Resume Gate
| 依赖 | 必须存在 | 不满足时处理 |
| --- | --- | --- |
| T05 | 前置 result JSON 和 HANDOFF，`none` 除外 | 读取 blockers/repair queue；不得猜测前置成功 |
...
## 输出文件
docs/auto-execute/results/T06.json; docs/auto-execute/external-data/T06/
