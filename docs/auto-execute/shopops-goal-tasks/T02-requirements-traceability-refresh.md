...
| Task 类型 | 需求矩阵 |
| 主验收面 | P0/P1 需求追踪矩阵刷新 |
| 为什么选择这个模板 | 本任务的主要目标是 P0/P1 需求追踪矩阵刷新，与 `TPL-REQ-MATRIX` 的验收面一致。 |
...
| 独立任务模板文件 | `C:\Users\linyanhui\.codex\skills\task-auto-execute\references\templates\TPL-REQ-MATRIX.md` |

## Codex Exec
```powershell
Set-Location -LiteralPath "D:\lyh\agent\agent-frame\ShopOps"
codex exec "Use auto-execute. Execute only D:\lyh\agent\agent-frame\ShopOps\docs\auto-execute\shopops-goal-tasks\T02-requirements-traceability-refresh.md. Treat this as one fresh task boundary. Do not stop after planning. Implement, test, repair, write result JSON, write HANDOFF, and exit. If blocked, classify the blocker in durable files and route to repair; do not end with only chat text."
```

## 实现功能
把所有需求映射到实现任务、验证任务和证据路径。
...
- 本任务结束必须写 `docs/auto-execute/results/T02.json`。
- 本任务结束必须写 `docs/auto-execute/latest/T02-HANDOFF.md`。
- result JSON 必须包含 `task_pack_id: "shopops-goal-2026-06-03"`、`task_id: "T02"`、`status`、`evidence`、`blockers`。
...
| 面 | 标准 | 证据路径 |
| --- | --- | --- |
| 需求 | `REQ-001..REQ-014` 每一项有实现/缺口判定 | `docs/auto-execute/results/T02.json` |
| 数据 | 相关表字段、unique_key、upsert、pending replay、readback 有证据或 blocker | `docs/auto-execute/external-data/T02/` |
| 测试 | 相关命令运行并记录输出，失败则本任务内修复或写 repair | `docs/auto-execute/logs/T02/` |
| 安全 | 不泄露密钥，不触碰真实投放副作用 | `docs/auto-execute/results/T02.json` |
...
| 依赖 | 必须存在 | 不满足时处理 |
| --- | --- | --- |
| T01 | 前置 result JSON 和 HANDOFF，`none` 除外 | 读取 blockers/repair queue；不得猜测前置成功 |
...
## 输出文件
docs/auto-execute/results/T02.json; shopops-goal-requirement-traceability-matrix.md
