# Task T05 - taobao-api-boundary-and-nonblocking-config
...
| Task 类型 | API 边界 |
| 主验收面 | 淘宝官方 API 后续切换边界，不阻塞当前 crawler MVP |
| 为什么选择这个模板 | 本任务的主要目标是 淘宝官方 API 后续切换边界，不阻塞当前 crawler MVP，与 `TPL-API-DOMAIN` 的验收面一致。 |
| 覆盖对象 | REQ-003 |
...
| 独立任务模板文件 | `C:\Users\linyanhui\.codex\skills\task-auto-execute\references\templates\TPL-API-DOMAIN.md` |

## Codex Exec
```powershell
Set-Location -LiteralPath "D:\lyh\agent\agent-frame\ShopOps"
codex exec "Use auto-execute. Execute only D:\lyh\agent\agent-frame\ShopOps\docs\auto-execute\shopops-goal-tasks\T05-taobao-api-boundary-and-nonblocking-config.md. Treat this as one fresh task boundary. Do not stop after planning. Implement, test, repair, write result JSON, write HANDOFF, and exit. If blocked, classify the blocker in durable files and route to repair; do not end with only chat text."
```

## 实现功能
保留 ORDER_SOURCE=api 接口、分页/付费过滤契约和凭据保护；当前不要求真实 API PASS。
...
## 允许改动范围
shopops/collectors/taobao_order_api.py; shopops/config.py; tests/**; docs/auto-execute/**
...
## 验收标准
- 覆盖对象 `REQ-003` 的实现或验证结果必须落到 durable files。
- 本任务结束必须写 `docs/auto-execute/results/T05.json`。
- 本任务结束必须写 `docs/auto-execute/latest/T05-HANDOFF.md`。
- result JSON 必须包含 `task_pack_id: "shopops-goal-2026-06-03"`、`task_id: "T05"`、`status`、`evidence`、`blockers`。
...
| 面 | 标准 | 证据路径 |
| --- | --- | --- |
| 需求 | `REQ-003` 每一项有实现/缺口判定 | `docs/auto-execute/results/T05.json` |
| 数据 | 相关表字段、unique_key、upsert、pending replay、readback 有证据或 blocker | `docs/auto-execute/external-data/T05/` |
| 测试 | 相关命令运行并记录输出，失败则本任务内修复或写 repair | `docs/auto-execute/logs/T05/` |
| 安全 | 不泄露密钥，不触碰真实投放副作用 | `docs/auto-execute/results/T05.json` |

## 测试与证据
```powershell
pytest api boundary; secret guard。
```

## Dependency And Resume Gate
| 依赖 | 必须存在 | 不满足时处理 |
| --- | --- | --- |
| T04 | 前置 result JSON 和 HANDOFF，`none` 除外 | 读取 blockers/repair queue；不得猜测前置成功 |
