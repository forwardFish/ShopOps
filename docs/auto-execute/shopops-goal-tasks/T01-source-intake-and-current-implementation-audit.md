...
| Task 类型 | 接管审计 |
| 主验收面 | 需求文档与现有实现差距审计 |
| 为什么选择这个模板 | 本任务的主要目标是 需求文档与现有实现差距审计，与 `TPL-INTAKE` 的验收面一致。 |
...
| 独立任务模板文件 | `C:\Users\linyanhui\.codex\skills\task-auto-execute\references\templates\TPL-INTAKE.md` |

## Codex Exec
```powershell
Set-Location -LiteralPath "D:\lyh\agent\agent-frame\ShopOps"
codex exec "Use auto-execute. Execute only D:\lyh\agent\agent-frame\ShopOps\docs\auto-execute\shopops-goal-tasks\T01-source-intake-and-current-implementation-audit.md. Treat this as one fresh task boundary. Do not stop after planning. Implement, test, repair, write result JSON, write HANDOFF, and exit. If blocked, classify the blocker in durable files and route to repair; do not end with only chat text."
```

## 实现功能
重新读取 PRD、开发文档、现有代码、上一轮结果，写出需求是否已实现的逐项审计。
...
- 本任务结束必须写 `docs/auto-execute/results/T01.json`。
- 本任务结束必须写 `docs/auto-execute/latest/T01-HANDOFF.md`。
- result JSON 必须包含 `task_pack_id: "shopops-goal-2026-06-03"`、`task_id: "T01"`、`status`、`evidence`、`blockers`。
...
| 面 | 标准 | 证据路径 |
| --- | --- | --- |
| 需求 | `REQ-001..REQ-014` 每一项有实现/缺口判定 | `docs/auto-execute/results/T01.json` |
| 数据 | 相关表字段、unique_key、upsert、pending replay、readback 有证据或 blocker | `docs/auto-execute/external-data/T01/` |
| 测试 | 相关命令运行并记录输出，失败则本任务内修复或写 repair | `docs/auto-execute/logs/T01/` |
| 安全 | 不泄露密钥，不触碰真实投放副作用 | `docs/auto-execute/results/T01.json` |
...
| 依赖 | 必须存在 | 不满足时处理 |
| --- | --- | --- |
| T00 | 前置 result JSON 和 HANDOFF，`none` 除外 | 读取 blockers/repair queue；不得猜测前置成功 |
...
## 输出文件
docs/auto-execute/results/T01.json; docs/auto-execute/shopops-goal-implementation-audit.md
