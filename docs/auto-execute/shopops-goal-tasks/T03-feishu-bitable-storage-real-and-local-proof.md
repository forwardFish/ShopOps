# Task T03 - feishu-bitable-storage-real-and-local-proof
...
| Task 类型 | 外部数据 |
| 主验收面 | 飞书多维表格中文字段、upsert、pending replay、读回 |
| 为什么选择这个模板 | 本任务的主要目标是 飞书多维表格中文字段、upsert、pending replay、读回，与 `TPL-EXTERNAL-DATA` 的验收面一致。 |
| 覆盖对象 | REQ-007,REQ-008,REQ-009,DATA-001..DATA-009 |
...
| 独立任务模板文件 | `C:\Users\linyanhui\.codex\skills\task-auto-execute\references\templates\TPL-EXTERNAL-DATA.md` |

## Codex Exec
```powershell
Set-Location -LiteralPath "D:\lyh\agent\agent-frame\ShopOps"
codex exec "Use auto-execute. Execute only D:\lyh\agent\agent-frame\ShopOps\docs\auto-execute\shopops-goal-tasks\T03-feishu-bitable-storage-real-and-local-proof.md. Treat this as one fresh task boundary. Do not stop after planning. Implement, test, repair, write result JSON, write HANDOFF, and exit. If blocked, classify the blocker in durable files and route to repair; do not end with only chat text."
```

## 实现功能
实现或修复真实 FeishuBitableStorage 与本地 fake 双证据；缺凭据时写 BLOCKED_BY_ENVIRONMENT。
...
## 允许改动范围
shopops/storage/**; shopops/config.py; tests/**; scripts/acceptance/**; docs/auto-execute/**
...
## 验收标准
- 覆盖对象 `REQ-007,REQ-008,REQ-009,DATA-001..DATA-009` 的实现或验证结果必须落到 durable files。
- 本任务结束必须写 `docs/auto-execute/results/T03.json`。
- 本任务结束必须写 `docs/auto-execute/latest/T03-HANDOFF.md`。
- result JSON 必须包含 `task_pack_id: "shopops-goal-2026-06-03"`、`task_id: "T03"`、`status`、`evidence`、`blockers`。
...
| 面 | 标准 | 证据路径 |
| --- | --- | --- |
| 需求 | `REQ-007,REQ-008,REQ-009,DATA-001..DATA-009` 每一项有实现/缺口判定 | `docs/auto-execute/results/T03.json` |
| 数据 | 相关表字段、unique_key、upsert、pending replay、readback 有证据或 blocker | `docs/auto-execute/external-data/T03/` |
| 测试 | 相关命令运行并记录输出，失败则本任务内修复或写 repair | `docs/auto-execute/logs/T03/` |
| 安全 | 不泄露密钥，不触碰真实投放副作用 | `docs/auto-execute/results/T03.json` |

## 测试与证据
```powershell
pytest storage tests; verify_feishu_data.py; real credential probe。
```

## Dependency And Resume Gate
| 依赖 | 必须存在 | 不满足时处理 |
| --- | --- | --- |
| T02 | 前置 result JSON 和 HANDOFF，`none` 除外 | 读取 blockers/repair queue；不得猜测前置成功 |
...
## 输出文件
docs/auto-execute/results/T03.json; docs/auto-execute/external-data/T03/
