...
def split_keywords(raw: str) -> list[str]:
    default_keyword = "\u6d17\u9762\u5976"
    return [item.strip() for item in (raw or default_keyword).replace("\uff0c", ",").split(",") if item.strip()]
...
def main() -> int:
    ...
    if rows and args.collection_mode == "profile":
        upload_summary = newest_summary(batch_root) or {"status": "already_uploaded_by_profile_crawler"}
    elif rows:
