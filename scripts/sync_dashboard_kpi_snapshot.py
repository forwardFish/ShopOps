...
def kpi_formulas(summary_table_name: str) -> dict[str, dict[str, str]]:
    ...
    return {
        ...
        "已知总投入": {"expression": "[投流消耗]+[达人佣金]", "formatter": "0.00"},
        "已知费用后利润": {"expression": "[有效销售额]-[商品成本]-[运费成本]-[平台扣点]-[其他费用]-[达人佣金]-[投流消耗]", "formatter": "0.00"},
        "投流后毛利": {"expression": "[有效销售额]-[达人佣金]-[投流消耗]", "formatter": "0.00"},
        "经营利润估算": {"expression": "[已知费用后利润]", "formatter": "0.00"},
        "ROI": {"expression": "IF([投流消耗]=0,IF([达人佣金]=0,0,[有效销售额]/[达人佣金]),[有效销售额]/[投流消耗])", "formatter": "0.00"},
        "平台ROI": {"expression": "IF([已知总投入]=0,0,[有效销售额]/[已知总投入])", "formatter": "0.00"},
        "已知费用利润率": {"expression": "IF([有效销售额]=0,0,[已知费用后利润]/[有效销售额])", "formatter": "0.00"},
        "利润率": {"expression": "IF([有效销售额]=0,0,[经营利润估算]/[有效销售额])", "formatter": "0.00"},
        ...
        "数据状态": {"expression": 'IF([源表记录数]=0,"partial",IF([订单数]=0,"partial",IF([投流记录数]=0&&[达人佣金]=0,"partial","normal")))', "formatter": ""},
        "缺失项": {"expression": 'IF([源表记录数]=0,"无源记录",IF([订单数]=0,IF([投流记录数]=0&&[达人佣金]=0,"订单,投入","订单"),IF([投流记录数]=0&&[达人佣金]=0,"投入","")))', "formatter": ""},
